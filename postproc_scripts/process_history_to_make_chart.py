...
for i in range(1, num_workers + 1):
    ...
    b = [(H[i]["gen_started_time"] - mint, H[i]["gen_ended_time"] - mint) for i in inds]
    gnt.broken_barh(b, (i, 0.9), facecolors=gen_work_color, edgecolor="k")

    inds = np.where(H["sim_worker"] == i)[0]
    b = [(H[i]["sim_started_time"] - mint, H[i]["sim_ended_time"] - mint) for i in inds]
    gnt.broken_barh(b, (i, 0.9), facecolors=sim_work_color, edgecolor="k")
