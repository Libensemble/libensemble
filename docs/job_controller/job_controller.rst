...
Jobs are created and returned through the job_controller launch function. Jobs
...
functions. Note that the job attributes are updated only when they are
...
:job.finished: (Boolean) True means job has finished running - not whether it was successful.
...
:job.name: (string) Name of job - autogenerated
...
A list of job_controller and job functions can be found under the ``job_controller`` module.
