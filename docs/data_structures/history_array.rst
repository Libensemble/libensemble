...
current scheduling status of the point (``sim_start`` when the sim evaluation
...
``sim_start`` [bool]: True if entry was given to sim_worker for sim evaluation

``sim_start_time`` [float]: Time entry was given to sim_worker for a sim evaluation
...
reserved fields: ``sim_id``, ``sim_start``, and ``sim_end`` are shown for brevity.
