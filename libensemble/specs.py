...
from libensemble.utils.type_checkers import (
    ...
    _check_output_fields, _MPICommField)
...
class LibeSpecs(BaseModel):
    ...
    mpi_comm: Optional[_MPICommField] = None  # see utils/type_checkers.py
...
class EnsembleSpecs(BaseModel):
    H0: np.ndarray = Field(default_factory=lambda: np.empty([0]))
