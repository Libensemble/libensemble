...
def only_persistent_workers(W, H, sim_specs, gen_specs, alloc_specs, persis_info):
    ...
    **User options**:
    ...
    # SH Follows same workflow as original but alternative is to just set workers up at the start.
    # SH Need to check can set up without sending data at start.
    # SH Make sure once set up persistent sim that it W array is correct.
    task_avail = ~H['given'] & ~H['cancel_requested']

    if np.any(task_avail):

        #for i in avail_worker_ids(W, persistent=EVAL_SIM_TAG):  # SH - if previously set up persistent sims.
        for i in avail_worker_ids(W, persistent=None):  #   SH This will send whether currently persistent or not (with data)

            # SH This is a kluge - but resolved in variable resource branch
            # SH Leave worker 1 for gen.
            if i == 1:
                continue
        ...
        # Finally, call a persistent generator on worker 1.
        gen_count += 1
        gen_work(Work, 1, gen_specs['in'], range(len(H)), persis_info.get(1),
                 persistent=True, active_recv=active_recv_gen)
        persis_info['gen_started'] = True
