...
def only_persistent_gens(W, H, sim_specs, gen_specs, alloc_specs, persis_info, libE_info):
    ...
    **User options**:
    ...
    for wid in avail_workers:
        ...
        try:
            Work[wid] = support.sim_work(
                wid, H, sim_specs["in"], sim_ids_to_send, persis_info.get(wid), persistent=user.get("alt_type", False)
            )
    ...
    # Start persistent gens if no work to give out. Uses zero_resource_workers if defined.
    if not np.any(points_to_evaluate):
