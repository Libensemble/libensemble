"""
Runs libEnsemble with APOSMM (direct implementation) + IBCDFO POUNDERS using
JAX-based combinemodels on the synthetic beamline problem.

Uses GenSpecs(generator=APOSMM(...)) with the legacy sim_f interface.

Execute via one of the following commands:
   mpiexec -np 3 python test_persistent_aposmm_ibcdfo_pounders_jax.py
   python test_persistent_aposmm_ibcdfo_pounders_jax.py --nworkers 2
"""
