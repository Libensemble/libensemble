...
for run in range(2):
    ...
    if api.is_manager:
        if run == 0:
            assert np.sum(api.H['last_points']) == 0, ("The last_points shouldn't be marked (even though "
                                                       "they were marked in the gen) as 'use_persis_return' was false.")
        elif run == 1:
            # import ipdb; ipdb.set_trace()
            assert np.sum(api.H['last_points']) == 100, ("The last_points should be marked as true because they "
