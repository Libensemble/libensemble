...
def create_H0(persis_info, gen_specs, sim_max):
    ...
    H0 = np.zeros(b, dtype=[('x', float, 2), ('sim_id', int), ('sim_start', bool), ('cancel_requested', bool)])
    ...
    # gen_specs['out'].append(('sim_start', bool))
...
for testnum in range(1, 6):
    ...
    if is_manager:
        ...
        assert np.all(~H['sim_start'][::10]), 'Some values are given that should not have been'
        tol = 0.1
        for m in minima:
