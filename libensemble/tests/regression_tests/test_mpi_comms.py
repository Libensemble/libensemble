...
), "This test can only be run with mpi comms -- aborting..."
...
def check_ranks(mpi_comm, test_exp, test_num):
    ...
    print(
        "got {},  exp {} ".format(comm_ranks_in_world, test_exp[test_num]), flush=True
    )
    # This is really testing the test is testing what is it supposed to test
    assert comm_ranks_in_world == test_exp[test_num], (
        "comm_ranks_in_world are: "
        + str(comm_ranks_in_world)
        + " Expected: "
        + str(test_exp[test_num])
    )
    if rank == -1:
        return False
    return True
