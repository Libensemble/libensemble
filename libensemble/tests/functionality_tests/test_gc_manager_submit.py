...
def _make_gc_executor(sim_f):
    """Return a mock GC executor whose submit_to_registered_function calls
    *sim_f* synchronously and wraps the result in a resolved Future."""
...
if __name__ == "__main__":
    ...
    SIM_MAX = 20
    N_VIRTUAL_WORKERS = 4  # max concurrent GC futures (no real workers needed)
    ...
    # Every completed point must have a non-negative norm
    assert np.all(completed["f"] >= 0.0), "Unexpected negative norm value"

    # Verify the executor was actually used (not bypassed)
    assert mock_executor.submit_to_registered_function.call_count >= SIM_MAX, (
        f"Expected at least {SIM_MAX} GC submissions, " f"got {mock_executor.submit_to_registered_function.call_count}"
    )
