...
def get_MPI_variant():
    ...
    try:
        subprocess.check_call(
            ["aprun", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        return "aprun"
    except OSError:
        pass
    ...
    try:
        subprocess.check_call(
            ["jsrun", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        return "jsrun"
    except OSError:
        pass
    ...
    try:
        ...
        try_mpich = subprocess.Popen(
            ["mpirun", "-npernode"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    try:
        subprocess.check_call(
            ["srun", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        return "srun"
    except OSError:
        pass
...
def task_partition(num_procs, num_nodes, procs_per_node, machinefile=None):
    ...
    if machinefile:
        if num_procs or num_nodes or procs_per_node:
            logger.warning(
                "Machinefile provided - overriding " "procs/nodes/procs_per_node"
            )
...
def get_resources(
    resources, num_procs=None, num_nodes=None, procs_per_node=None, hyperthreads=False
):
    ...
    cores_avail_per_node = (
        gresources.logical_cores_avail_per_node
        if hyperthreads
        else gresources.physical_cores_avail_per_node
    )
    ...
    rassert(
        ...
        "Uneven distribution of node resources not yet supported. Nodes and slots are: {}".format(
            wresources.slots
        ),
    )
    ...
    rassert(
        ...
        "Requested {}. Only {} available".format(num_nodes, local_node_count),
    )
    ...
    if gresources.enforce_worker_core_bounds:
        rassert(
            ...
            "Requested {}. Only {} available".format(
                procs_per_node, cores_avail_per_node
            ),
        )
    ...
    if num_nodes < local_node_count:
        logger.warning(
            ...
            "than available. {} nodes used. {} nodes available".format(
                num_nodes, local_node_count
            )
        )
...
def create_machinefile(
    resources, machinefile=None, num_nodes=None, procs_per_node=None
):
    ...
    logger.debug(
        "Creating machinefile with {} nodes and {} ranks per node".format(
            num_nodes, procs_per_node
        )
    )
    ...
    return built_mfile, num_nodes, procs_per_node
