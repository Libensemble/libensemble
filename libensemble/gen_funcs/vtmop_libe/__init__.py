...
_ordered_dependencies = ['blas.f', 'shared_modules.f90', 'qnstop.f90', 'lapack.f', 'linear_shepard.f90', 'slatec.f', 'delsparse.f90', 'vtmop.f90', 'vtmop_libe.f90', 'vtmop_libe_c_wrapper.f90']
# 
# --------------------------------------------------------------------
#               AUTO-COMPILING
#
...
class vtmop_libe_mod:
    ...
    def vtmop_libe_init(self, d, p, lb, ub, lopt_budget, decay, des_tol, eps, epsw, obj_tol, min_radf, trust_radf, obj_bounds, pmode):
...
!'''
        ...
        # Setting up "p"
        if (type(p) is not ctypes.c_int): p = ctypes.c_int(p)
        
        # Setting up "lb"
        if ((not issubclass(type(lb), numpy.ndarray)) or
            (not numpy.asarray(lb).flags.f_contiguous) or
            (not (lb.dtype == numpy.dtype(ctypes.c_double)))):
            import warnings
            warnings.warn("The provided argument 'lb' was not an f_contiguous NumPy array of type 'ctypes.c_double' (or equivalent). Automatically converting (probably creating a full copy).")
            lb = numpy.asarray(lb, dtype=ctypes.c_double, order='F')
        lb_dim_1 = ctypes.c_int(lb.shape[0])
        
        # Setting up "ub"
        if ((not issubclass(type(ub), numpy.ndarray)) or
            (not numpy.asarray(ub).flags.f_contiguous) or
            (not (ub.dtype == numpy.dtype(ctypes.c_double)))):
            import warnings
            warnings.warn("The provided argument 'ub' was not an f_contiguous NumPy array of type 'ctypes.c_double' (or equivalent). Automatically converting (probably creating a full copy).")
        ...
        if (type(lopt_budget) is not ctypes.c_int): lopt_budget = ctypes.c_int(lopt_budget)
        
        # Setting up "decay"
        if (type(decay) is not ctypes.c_double): decay = ctypes.c_double(decay)
        
        # Setting up "des_tol"
        if (type(des_tol) is not ctypes.c_double): des_tol = ctypes.c_double(des_tol)
        
        # Setting up "eps"
        if (type(eps) is not ctypes.c_double): eps = ctypes.c_double(eps)
        
        # Setting up "epsw"
        if (type(epsw) is not ctypes.c_double): epsw = ctypes.c_double(epsw)
        
        # Setting up "obj_tol"
        if (type(obj_tol) is not ctypes.c_double): obj_tol = ctypes.c_double(obj_tol)
        
        # Setting up "min_radf"
        if (type(min_radf) is not ctypes.c_double): min_radf = ctypes.c_double(min_radf)
        
        # Setting up "trust_radf"
        if (type(trust_radf) is not ctypes.c_double): trust_radf = ctypes.c_double(trust_radf)
        
        # Setting up "obj_bounds"
        if ((not issubclass(type(obj_bounds), numpy.ndarray)) or
            (not numpy.asarray(obj_bounds).flags.f_contiguous) or
            (not (obj_bounds.dtype == numpy.dtype(ctypes.c_double)))):
            import warnings
            warnings.warn("The provided argument 'obj_bounds' was not an f_contiguous NumPy array of type 'ctypes.c_double' (or equivalent). Automatically converting (probably creating a full copy).")
        ...
        # Setting up "pmode"
        if (type(pmode) is not ctypes.c_bool): pmode = ctypes.c_bool(pmode)
    
        # Call C-accessible Fortran wrapper.
        clib.c_vtmop_libe_init(ctypes.byref(d), ctypes.byref(p), ctypes.byref(lb_dim_1), ctypes.c_void_p(lb.ctypes.data), ctypes.byref(ub_dim_1), ctypes.c_void_p(ub.ctypes.data), ctypes.byref(ierr), ctypes.byref(lopt_budget), ctypes.byref(decay), ctypes.byref(des_tol), ctypes.byref(eps), ctypes.byref(epsw), ctypes.byref(obj_tol), ctypes.byref(min_radf), ctypes.byref(trust_radf), ctypes.byref(obj_bounds_dim_1), ctypes.byref(obj_bounds_dim_2), ctypes.c_void_p(obj_bounds.ctypes.data), ctypes.byref(pmode))
...
!'''
        ...
        # Setting up "des_pts"
        if ((not issubclass(type(des_pts), numpy.ndarray)) or
            (not numpy.asarray(des_pts).flags.f_contiguous) or
            (not (des_pts.dtype == numpy.dtype(ctypes.c_double)))):
            import warnings
            warnings.warn("The provided argument 'des_pts' was not an f_contiguous NumPy array of type 'ctypes.c_double' (or equivalent). Automatically converting (probably creating a full copy).")
        ...
        # Setting up "obj_pts"
        if ((not issubclass(type(obj_pts), numpy.ndarray)) or
            (not numpy.asarray(obj_pts).flags.f_contiguous) or
            (not (obj_pts.dtype == numpy.dtype(ctypes.c_double)))):
            import warnings
            warnings.warn("The provided argument 'obj_pts' was not an f_contiguous NumPy array of type 'ctypes.c_double' (or equivalent). Automatically converting (probably creating a full copy).")
        ...
        # Setting up "isnb"
        if (type(isnb) is not ctypes.c_int): isnb = ctypes.c_int(isnb)
        
        # Setting up "snb"
        if (type(snb) is not ctypes.c_int): snb = ctypes.c_int(snb)
        
        # Setting up "onb"
        if (type(onb) is not ctypes.c_int): onb = ctypes.c_int(onb)
        ...
        # Call C-accessible Fortran wrapper.
        clib.c_vtmop_libe_generate(ctypes.byref(d), ctypes.byref(p), ctypes.byref(lb_dim_1), ctypes.c_void_p(lb.ctypes.data), ctypes.byref(ub_dim_1), ctypes.c_void_p(ub.ctypes.data), ctypes.byref(des_pts_dim_1), ctypes.byref(des_pts_dim_2), ctypes.c_void_p(des_pts.ctypes.data), ctypes.byref(obj_pts_dim_1), ctypes.byref(obj_pts_dim_2), ctypes.c_void_p(obj_pts.ctypes.data), ctypes.byref(isnb), ctypes.byref(snb), ctypes.byref(onb), ctypes.byref(obj_bounds_dim_1), ctypes.byref(obj_bounds_dim_2), ctypes.c_void_p(obj_bounds.ctypes.data), ctypes.byref(lbatch), ctypes.byref(batchx_dim_1), ctypes.byref(batchx_dim_2), ctypes.byref(batchx), ctypes.byref(ierr))
