...
def _dump_on_abort(hist, persis_info, save_H=True):
    ...
    logger.error("Dumping ensemble history with {} sims evaluated:".
                 format(hist.sim_end_count))

    if save_H:
        np.save('libE_history_at_abort_' + str(hist.sim_end_count) + '.npy', hist.trim_H())
        with open('libE_persis_info_at_abort_' + str(hist.sim_end_count) + '.pickle', "wb") as f:
