  ...
  \begin{description}
    ...
    \item[allocation\_specs]: [dict]
      \begin{description}
        ...
        \item[worker\_ranks]: [python set of ints]
        \item[machinefile]:
      \end{description}
    ...
    \item[sim\_specs]: [dict] 
      \begin{description}
        ...
        \item[out]: [dict] string keys and integer/tuple sizes of sim outputs (assumed common to all sims)
    ...
    \item[gen\_specs]: [dict] 
      \begin{description}
        ...
        \item[out]: [dict] string keys and integer/tuple sizes of gen outputs
...
\begin{allintypewriter}
  out = sim\_f(x, params)
\end{allintypewriter}
where \texttt{out} is a dictionary with keys/value-sizes matching those in \texttt{sim\_specs[out]}.
...
\clearpage
\subsection{Example of calling function}
\lstinputlisting[basicstyle=\footnotesize,frame=single,language=Python,title=\lstname,numberstyle=\tiny,numbers=left,firstline=36]{../../examples/GKLS/call_libE_on_GKLS.py}
...
\begin{enumerate}
  ...
  \item 
    \begin{description}
      \item[Objective:] chiwrt\{1,2\}.c with stochastic noise on each residual
