...
% \begin{algorithm}[H] % Makes font larger
  ...
  \SetKw{true}{true}
  \SetKw{false}{false}
  \Input{Simulations $\mathtt{sim_1,\ldots,sim_{n_s}}$, \newline
  ...
  Initialize worker $i$ on $\mathtt{gen}(H,A,Q)$\\
  }
  
  \While{$\mathtt{term\_test}(H,A,Q)$}
  {
    $(A,Q) = \mathtt{update\_active\_and\_queue}(H,A,Q)$ \Comment*[r]{blocks manager}
