...
- **Mixed sim/gen interfaces**: It is valid to use a gest-api generator (`GenSpecs(generator=...)`) together with a legacy sim function (`SimSpecs(sim_f=..., inputs=[...], outputs=[...])`). This is useful when the simulator returns complex structured array fields (e.g., multi-component `fvec`) that cannot be naturally expressed as named VOCS observables. The generator's `variables_mapping` must include entries for any extra fields the sim returns that the gen needs (e.g., `"fvec": ["fvec"]`).
- **`multiprocessing.set_start_method`**: Scripts that use generators or simulators spawning subprocesses (e.g., local optimizers via `LocalOptInterfacer`) must call `multiprocessing.set_start_method("fork", force=True)` on Linux/macOS before any forking occurs. This should be placed before the `Ensemble(parse_args=True)` call inside an `if __name__ == "__main__":` block.

Test Writing Guidelines
-----------------------

- **Use `Ensemble` + dataclasses for regression tests**: Prefer `Ensemble(parse_args=True)` with `GenSpecs`, `SimSpecs`, and `ExitCriteria` dataclasses over the legacy `libE()` function and dict-based specs. The `Ensemble` class handles argument parsing, comms setup, and output automatically.
- **`TESTSUITE_NPROCS` counts manager + workers**: The `# TESTSUITE_NPROCS: N` comment sets the total MPI rank count. With gest-api generators running on the manager thread, all `N-1` remaining processes are workers available for simulation tasks — so fewer total processes are needed than with a persistent generator occupying its own worker.
- **Unit tests call generators directly**: Unit tests for `LibensembleGenerator` subclasses can call `suggest_numpy`/`ingest_numpy` (or `suggest`/`ingest`) directly without spinning up a libEnsemble manager-worker loop. This is much faster and simpler than regression tests.
- **`variables_mapping` is the translation layer**: When a `LibensembleGenerator` subclass uses internal numpy field names (e.g., `x`, `f`, `x_on_cube`) but the user-facing VOCS uses named variables, `variables_mapping` maps between them. The format is `{"internal_name": ["vocs_var1", "vocs_var2"]}`. Multi-dimensional internal fields (e.g., `x` of shape `(n,)`) map to `n` individual scalar VOCS variables. The `unmap_numpy_array` utility performs the reverse direction (internal → VOCS).
- **`@pytest.mark.extra` for slow tests**: Slow unit tests that invoke actual optimization loops should be marked `@pytest.mark.extra` so they are skipped in fast test runs and only run in the full CI suite.
- **Doc `literalinclude` paths**: Documentation in ``docs/`` uses relative paths from the doc file to source files. When test files are renamed or moved, update any corresponding `.. literalinclude::` directives in `.rst` files under ``docs/``.
