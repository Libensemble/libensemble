...
sim_specs = {
    ...
    "out": [("f", float), ("fvec", float, m), ("error", float)],  # Name, type of output from sim_f, fix velocity_field to error (last error value)
}
...
H0 = np.zeros(1, dtype = [('x', float, 3), ('x_on_cube', float, 3), ('f', float), ('fvec', float, m), ('sim_id', int), ('local_pt', bool), ('dist_to_better_l', float), ('dist_to_better_s', float) ])
H0['x'] = np.array([0.2, 0.1, 0.05])
H0['x_on_cube'] = (H0['x']-gen_specs["user"]["lb"])/(gen_specs["user"]["ub"]-gen_specs["user"]["lb"])
H0['fvec'] = np.random.uniform(0,1,m)
