...
sim_specs = {
    ...
    "out": [("f", float), ("error", float)],  # Name, type of output from sim_f, fix velocity_field to error (last error value)
}
...
exit_criteria = {"sim_max": 30}  # exit_criteria = {"sim_max": 8}

# Seed random streams for each worker, particularly for gen_f
persis_info = add_unique_random_streams({}, nworkers + 1)

H0 = np.zeros(1, dtype = [('x', float, 3), ('x_on_cube', float, 3), ('f', float), ('sim_id', int), ('local_pt', bool), ('dist_to_better_l', float), ('dist_to_better_s', float) ])
